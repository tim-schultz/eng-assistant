...
use crate::conversation_manager::ConversationStorage;
use crate::conversation_manager::CurrentConversation;
...
pub static TOOLS: Lazy<Value> = Lazy::new(|| {
    json!([
        ...
        },
        {
            ...
            "description": "Compile a rust project at the specified path. Use this when you want to determine if changes to a rust project build successfully. It will return the compilation results.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "The path of the cargo crate to compile"
                    }
                },
                "required": ["path"]
            }
...
impl ToolExecutor {
    ...
    pub fn search_file(&self, path: &str, search_pattern: &str) -> Result<String> {
        let content = fs::read_to_string(path)?;
        let mut results = Vec::new();

        for (line_number, line) in content.lines().enumerate() {
            if line.contains(search_pattern) {
                results.push(format!("Line {}: {}", line_number + 1, line));
            }
        }
        ...
        match tool_name {
            ...
            "search_file" => self.search_file(
                ...
                tool_input["search_pattern"]
                    .as_str()
                    .ok_or(anyhow!("Missing search_pattern"))?,
            ),
            "edit_and_apply" => {
                self.edit_and_apply(
                    tool_input["path"].as_str().ok_or(anyhow!("Missing path"))?,
            ...
            "read_multiple_files" => {
                let paths = tool_input["paths"]
                    .as_array()
                    .ok_or(anyhow!("Missing paths array"))?
                    .iter()
            ...
            "compile_rust_project" => self
                .compile_rust_project(tool_input["path"].as_str().ok_or(anyhow!("Missing path"))?),
            _ => Err(anyhow!("Unknown tool: {}", tool_name)),
        }
    }

    pub fn compile_rust_project(&self, project_path: &str) -> Result<String> {
        if !self.list_files(project_path)?.contains("Cargo.toml") {
            return Err(anyhow!(
                "Path does not include Cargo.toml: {}",
                project_path
            ));
        }
        ...
        // For successful builds, return combined output
    ...
    pub fn highlight_diff(&self, diff_text: &str) -> String {
    ...
    pub async fn parse_search_replace_blocks(&self, text: &str) -> Result<String> {
        let re =
    ...
    pub fn generate_diff(&self, old: &str, new: &str, file_path: &str) -> Result<String> {
    ...
    pub fn read_multiple_files(&self, paths: Vec<String>) -> Result<String> {
        let mut results = Vec::new();
        for path in paths {
            match self.read_file(&path) {
                ...
                Err(e) => {
                    ...
                    results.push(format!("Failed to read file {}: {}", path, e));
                }
            }
        }
        Ok(results.join("\n"))
...
"#,
        ...
        let result = executor
            .compile_rust_project(temp_dir.path().to_str().unwrap())
            .unwrap();
...
"#,
        ...
        fs::write(
            temp_dir.path().join("src").join("lib.rs"),
            "pub fn add(a: i32, b: i32) -> i32 { a + c }",
        )
        .unwrap();
        ...
        let result = executor
            ...
            .unwrap();
        dbg!(&result);
        assert!(result.contains("cannot find value `c` in this scope"));
    }
    ...
    fn test_compile_rust_project_missing_cargo_toml() {
        ...
        let result = executor.compile_rust_project(temp_dir.path().to_str().unwrap());
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Path does not include Cargo.toml"));
    }
